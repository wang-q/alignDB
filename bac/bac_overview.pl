...
# running options
my $overview_file = $Config->{bac}{overview_file};
...
my $species = sub {
    ...
    {    # write header
        my @headers = qw{ genus_id genus species_id species chr_number
...
my $gc_checklist = sub {
    ...
    {    # write header
        my @headers = qw{
            genus_id genus species_id species avg_genome_size avg_gc count
        };
        push @headers, "check\ntable", "check\ntree", "check\nalign",
            "check\nxlsx";
        ( $sheet_row, $sheet_col ) = ( 0, 0 );
    ...
    {    # write contents
        ...
        my $sql_query = q{
            SELECT  genus_id,
                    ...
                    count(*) count
            FROM strain
            WHERE   species_member > 2
                    and seq_ok = 1
                    and super_kingdom = 'bacteria'
            GROUP BY species_id
            ORDER BY species
        };
        my %option = (
